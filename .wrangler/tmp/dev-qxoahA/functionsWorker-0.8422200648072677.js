...
// .wrangler/tmp/pages-Cts5fP/functionsWorker-0.8422200648072677.mjs
...
var NEWS_FEEDS = [
  {
    ...
    url: "https://news.google.com/rss/search?q=site%3Anews24.com%20South%20Africa&hl=en-ZA&gl=ZA&ceid=ZA%3Aen"
  },
  {
    source: "News24",
    category: "Technology",
    url: "https://news.google.com/rss/search?q=site%3Anews24.com%20technology%20OR%20fintech%20OR%20artificial%20intelligence&hl=en-ZA&gl=ZA&ceid=ZA%3Aen"
  },
  {
    source: "UNDP",
    category: "Development",
    url: "https://news.google.com/rss/search?q=site%3Aundp.org%20Africa%20OR%20South%20Africa%20OR%20digital%20development&hl=en-ZA&gl=ZA&ceid=ZA%3Aen"
  }
];
async function onRequestGet() {
  try {
    ...
    return Response.json(
      {
        success: false,
        message: "The intelligence feed is temporarily unavailable.",
        generatedAt: (/* @__PURE__ */ new Date()).toISOString(),
        articles: []
      },
      {
...
__name2(fetchNewsFeed, "fetchNewsFeed");
function parseRssFeed(xml, feed) {
  const itemBlocks = xml.match(/<item\b[\s\S]*?<\/item>/gi) || [];
  return itemBlocks.slice(0, 15).map(function(item) {
    const rawTitle = cleanText(
      getTagValue(item, "title")
    );
    const rawDescription = getTagValue(item, "description");
    const description = cleanText(rawDescription);
    const image = getMediaImage(item) || getImageFromDescription(rawDescription) || null;
    return {
...
function getTagValue(xmlBlock, tagName) {
  ...
  const match2 = xmlBlock.match(expression);
  return match2 ? match2[1] : "";
}
__name(getTagValue, "getTagValue");
__name2(getTagValue, "getTagValue");
...
__name(getMediaImage, "getMediaImage");
__name2(getMediaImage, "getMediaImage");
function getImageFromDescription(description) {
  const decodedDescription = decodeXml(description);
  const match2 = decodedDescription.match(
    /<img[^>]+src=["']([^"']+)["']/i
  );
  return match2 ? normaliseImageUrl(match2[1]) : null;
}
__name(getImageFromDescription, "getImageFromDescription");
__name2(getImageFromDescription, "getImageFromDescription");
...
__name(normaliseImageUrl, "normaliseImageUrl");
__name2(normaliseImageUrl, "normaliseImageUrl");
function cleanDescription(description, title) {
  let result = String(description || "");
  result = result.replace(title, "").replace(/\s+-\s+[A-Za-z0-9 .&'-]+$/, "").replace(/\s+/g, " ").trim();
  return result;
}
__name(cleanDescription, "cleanDescription");
__name2(cleanDescription, "cleanDescription");
...
function cleanHeadline(title) {
  return String(title || "").replace(/\s+-\s+News24\s*$/i, "").replace(/\s+-\s+UNDP\s*$/i, "").replace(
    /\s+-\s+United Nations Development Programme\s*$/i,
    ""
  ).trim();
}
__name(cleanHeadline, "cleanHeadline");
__name2(cleanHeadline, "cleanHeadline");
...
__name2(removeDuplicates, "removeDuplicates");
var routes = [
  {
    routePath: "/api/news",
    mountPath: "/api",
...
// .wrangler/tmp/bundle-D15CT3/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-D15CT3/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.8422200648072677.js.map
